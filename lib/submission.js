...
class Submission {
    ...
    async handleMessage(data, isBinary) {
        ...
        try {
            ...
            switch (code) {
                ...
                case "4":
                    if (!this.user) this.close("未登录易班");
                    const fullForm = await this.getFullForm("19af8564faf3d975fadca19f09bbf594")
                    this.data("newForm", {
                        wfid: "19af8564faf3d975fadca19f09bbf594",
                        form: fullForm.data.data
                    });
                    break;
                case "5":
                    break;
    ...
    async checkUpload(data) {
        ...
        const {
            uploadType = "direct", name = null, size = 0, type = ""
        } = data;
        try {
            ...
            if (uploadType == "url") {
                ...
                const getFile = await fetch(file, data.url);
                if (!getFile.ok) {
                ...
                this.upload({
                    name: name,
                    type: getFile.headers["content-type"],
                    size: getFile.headers["content-length"],
    ...
    async getCompleteTask(wfid = 0) {
        ...
        this.data("lastSubmit", {
            ok: false,
            message: "没有获取到上一次打卡提交的数据。可能是提供了错误的工作流id"
        });
    };
    //提交任务
    ...
    async upload(file = {
        ...
        this.message("开始向易班上传:", file.name);
        const result = {
            ok: false,
        };
        try {
            if (![...allowedFile, ...allowedImage].includes(file.type)) throw new Error("不允许上传的文件类型: " + file.type);
            if (file.size >= maxImgSize) throw new Error(`文件大小不合规: 应大于0B，小于 ${maxImgSize} B`);
            const isImage = allowedImage.includes(file.type);
            const uploadBaseUrl = (isImage) ? "https://api.uyiban.com/workFlow/c/my/getUploadUrl" : "https://api.uyiban.com/workFlow/c/my/getUploadUriForAttachment";
            const queryStr = `name=${encodeURIComponent(file.name)}&type=${encodeURIComponent(file.type)}&size=${file.size}&CSRF=${this.csrf}`;
            ...
            result.message = "成功上传文件至易班: " + file.name;
            result.ok = true;
        } catch (err) {
