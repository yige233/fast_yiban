...
import { maxImgSize, allowedImage, allowedFile } from "../config.js";

const symbol_id = Symbol("id");

class Submission {
  events = new Map([["checkUpload", []]]);
  ...
  async checkUpload(data) {
    ...
    const { uploadType = "direct", name = null, size = 0, type = "", component = "Attachment" } = data;
    try {
      ...
      if (uploadType == "direct") {
        ...
        for (let eventListener of this.events.get("checkUpload"))
          eventListener({
            token: token,
            data: {
              name: name,
  ...
  //登录易班
  async auth(data) {
    const { account = 0, passwd = null, rid = null, keysTime = null, cookies = null } = data;
    try {
      if (!account || !passwd) throw new Error("请提供账号和密码！");
      if (!rid || !keysTime || !cookies) throw new Error("需要通过人机验证！");
      ...
      const doLogin = await fetch("https://www.yiban.cn/login/doLoginAjax", {
        ...
        body: `account=${account}&password=${passwd}&keysTime=${keysTime}&rid=${rid}`,
      });
  ...
  async getAllTasks() {
    ...
    try {
      ...
      for (let task of json.data) {
        ...
        if (task.StartTime <= new Date() / 1e3)
          result.data.push({
            taskId: task.TaskId,
            title: task.Title,
          });
  ...
  async share(shareId) {
    return await fetch(`https://api.uyiban.com/workFlow/c/work/share?InitiateId=${shareId}&Action=view&Key=&CSRF=${this.csrf}`, {
      ...
      .then(res => res.json())
      .then(json => `https://app.uyiban.com/workflow/client/#/share?Key=${json.data.key}`);
  }
  ...
  //上传文件
  async upload(file) {
    const { component = "attachment", name = "", type = "", size = 0, data = new Blob() } = file;
