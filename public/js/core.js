...
class Client {
  ...
  remove(event, callback) {
    ...
    callbacks.splice(
      callbacks.findIndex(i => i == callback),
      1
    );
  }
  emit(event, data) {
  ...
  async login(account, passwd) {
    const { keysTime = null, cookies = null, rid = null } = await this.captchaChallenge();
    //登录
    if (!account || !passwd) {
      this.emit("message", "无法登录易班: 请提供账号和密码");
      return false;
    }
    if (!rid || !keysTime || !cookies) {
      this.emit("message", "无法登录易班: 请通过人机验证");
      return false;
    }
    ...
    const { ok, message } = await this.waitEvent("auth");
  ...
  //弹出人机验证。后端访问https://www.yiban.cn/login，并获取两个cookie：https_waf_cookie和YB_SSID，以及一个页面加载时间戳。人机验证获取rid后再把所有数返回后端。
  //后端解析html获取页面加载时间比较麻烦，而登录有要求必须使用这个时间戳，所以干脆送到前端让前端拿数据好了（懒 cookie也是不太方便传到对应的client，干脆让它走两遍（都是没法跨域的错
  async captchaChallenge() {
    const config = {
      organization: "IY3HadrRSlgwwKWo63gi",
      product: "popup",
      appId: "Yiban_Web",
      mode: "select",
      width: 300,
    };
    const loginPage = await fetch("/api/yibanLoginPage");
    const loginPageDom = new DOMParser().parseFromString(await loginPage.text(), "text/html");
    if (!window.initSMCaptcha) {
      await new Promise(resolve => {
        const js = document.createElement("script");
        js.src = "https://castatic.fengkongcloud.cn/pr/v1.0.4/smcp.min.js";
        js.onload = () => resolve();
        document.head.append(js);
      });
    }
    return {
      keysTime: loginPageDom.querySelector("#login-pr").dataset.keysTime,
      cookies: loginPage.headers.get("x-cookie-data"),
      rid: await new Promise((resolve, reject) => {
        initSMCaptcha(config, instance => {
          instance.onReady(() => instance.verify());
          instance.onSuccess(data => {
            if (data.pass) resolve(data.rid);
          });
          instance.onError(() => reject(null));
          instance.onClose(() => reject(null));
        });
      }),
    };
  }
  submit(wfid = this.wfid, rawData = this.rawData) {
    //提交表单
  ...
  dataEx(id, type, data = "") {
    //添加额外的表单数据。打卡表单的内容可能不会每次都相同，有变化的部分可以通过额外数据记录，自动打卡时，根据该记录实时生成打卡内容。
    switch (type) {
      case "time": //目前只有定位用到时间
  ...
  addFile(id, file) {
    //添加文件。此时并未真正添加文件，而是维护一个文件Map
    this.files[id] = file;
  }
  async import(data) {
    //导入数据，并自动执行打卡
    const { user, wfid, rawData } = data;
  ...
  get export() {
    ...
    for (let id in this.rawData) {
      //manual为false，就删除manual属性，减小数据大小
