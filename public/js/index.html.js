...
class Submit {
  ...
  loadjson(res) {
    for (let el of document.querySelector("ul").children)
      el.getAttribute("blocktype") && el.remove();
    const loadMap = (id) => {
      ...
      map.on("click", (e) => {
        ...
        new this.AMap.Geocoder({
          ...
          map.add(marker);
          the_marker = marker;
          if (container.getAttribute("prop") == "AutoTakePosition")
          ...
          if (container.getAttribute("prop") == "GdMap")
            container.setAttribute(
              "data",
              JSON.stringify({
          ...
          container.previousElementSibling.previousElementSibling.innerHTML =
            "获取定位：" + result.regeocode.formattedAddress;
        });
      });
    ...
    for (let form of json_.Form) {
      ...
      let id = form.id,
        ...
        br = document.createElement("br");
      el.id = id;
      desc.textContent = `${props.required ? "必须项：" : ""}${
  ...
  buildjson(id) {
    ...
    let type = el.getAttribute("blocktype")
      ? el.getAttribute("blocktype")
      : null;
    switch (type) {
      case "autoposition":
  ...
  async postdata(url) {
    ...
    try {
        ...
        case 3:
          ...
          history.pushState(
            ...
            window.location.origin +
              ...
              "?sign_data=" +
              encodeURIComponent(tool.b64en(JSON.stringify(this.fulljson)))
          );
          break;
        default:
          break;
      ...
      tool.showinfo([["text", "处理数据时出现问题，5分钟后重试。"]]);
...
const app = {
      ...
      .then((text) => {
        ...
        document.querySelector("#sendData").addEventListener("click", (e) => {
          ...
          app
            .send()
            .finally(() =>
              document.querySelector("#sendData").removeAttribute("disabled")
            );
        });
      })
      ...
      try {
        ...
        history.pushState(
          ...
          window.location.origin +
            window.location.pathname +
            "?sign_data=" +
            encodeURIComponent(tool.b64en(JSON.stringify(data)))
        );
        tool.showinfo([
          [
            "text",
            "sign_data的数据格式不正确。将于5秒钟后以更正格式的数据重载。",
          ],
        ]);
        setTimeout(() => {
          location.reload();
  ...
  send: async function () {
    ...
    if (
      ...
      return tool.showinfo([["text", "没有填入账号密码！"]]);
    this.submit.addaccount(
      document.querySelector("#account").value,
      document.querySelector("#passwd").value
    );
    if (lis.length > 1) {
      for (let li of lis) this.submit.buildjson(li.id);
      document.querySelector("#sign_data").value = tool.b64en(
        JSON.stringify(this.submit.fulljson)
      );
    } else if (document.querySelector("#sign_data").value) {
      let post = JSON.parse(
        tool.b64de(
          decodeURIComponent(document.querySelector("#sign_data").value)
        )
      );
